    ...
    : ComponentBase(config, context)
{
    ...
    if (config["use_kafka"].As<bool>(false)) {
        auto& producer_component =
            context.FindComponent<userver::kafka::ProducerComponent>("kafka-producer-folder");
        producer_ = &producer_component.GetProducer();
    }
}
...
{
    if (auto cached = folders_cache_.Get(owner_id)) {
        for (const auto& f : *cached) {
            if (f.name == name) return std::nullopt;
        }
    } else if (repository_->FolderNameExists(owner_id, name)) {
        return std::nullopt;
    }
    ...
    if (producer_) {
        producer_->Send("folder-events", folder.id, FolderToKafkaPayload(folder));
    } else {
        repository_->SaveFolder(folder);
    }

    {
        auto base = folders_cache_.Get(owner_id)
                        ? *folders_cache_.Get(owner_id)
                        : repository_->FindFoldersByOwner(owner_id);
        base.push_back(folder);
        folders_cache_.Set(owner_id, base);
    }
    return folder;
}
...
std::string FileSystemService::FolderToKafkaPayload(const models::Folder& folder) {
    userver::formats::json::ValueBuilder b;
    b["id"]              = folder.id;
    b["name"]            = folder.name;
    b["owner_id"]        = folder.owner_id;
    b["created_at_unix"] = std::chrono::duration_cast<std::chrono::seconds>(
                               folder.created_at.time_since_epoch()).count();
...
properties:
    ...
    use_kafka:
        type: boolean
        description: Publish FolderCreated events to Kafka instead of direct DB write
)");
}
